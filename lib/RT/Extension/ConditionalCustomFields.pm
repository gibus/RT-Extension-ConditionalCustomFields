...
This plugin provides the ability to display/edit a L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – called the "conditioned by L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>" throughout this documentation – conditioned by the value of another L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – the "condition L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>" – for the same object, which can be anything that can have custom fields (L<ticket|https://docs.bestpractical.com/rt/5.0.9/RT/Ticket.html>, L<queue|https://docs.bestpractical.com/rt/5.0.9/RT/Queue.html>, L<user|https://docs.bestpractical.com/rt/5.0.9/RT/User.html>, L<group|https://docs.bestpractical.com/rt/5.0.9/RT/Group.html>, L<article|https://docs.bestpractical.com/rt/5.0.9/RT/Article.html> or L<asset|https://docs.bestpractical.com/rt/5.0.9/RT/Asset.html>).

The condition can be setup on the Admin page for editing the conditioned by L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>. From version 0.99, any L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> can be chosen as the condition L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> (whereas for earlier version, only C<Select> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> were eligible), and you can specify which operator is to be applied against which value(s) for the condition to be met.
...
The condition is met if and only if the current value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/ObjectCustomField.html> is equal to the value (or one of the values, see below for multivalued condition) setup for this condition. With C<isn't> operator described below, C<is> operator is the only one which is eligible for selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – with C<Select>, C<Combobox> or C<Autocomplete> type –, since their values are to be selected from a set of values. For C<Date> and C<DateTime> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, this operator is named C<on>.

=item * C<isn't>

The condition is met if and only if the current value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/Queue.html> is different from the value (or none of the values, see below for multivalued condition) setup for this condition. With C<is> operator described above, C<isn't> operator is the only one which is eligible for selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – with C<Select>, C<Combobox> or C<Autocomplete> type –, since their values are to be selected from a set of values. For C<Date> and C<DateTime> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, this operator is named C<not on>.

=item * C<match>

The condition is met if and only if the current value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/ObjectCustomField.html> is included in the value setup for this condition, typically if the current value is a substring of the condition value. As said above, selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – with C<Select>, C<Combobox> or C<Autocomplete> type are not eligible for this operator. Also, C<Date> and C<DateTime> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> are not eligible for this operator.

=item * C<doesn't match>

The condition is met if and only if the current value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/ObjectCustomField.html> isn't included in the value setup for this condition, typically if the current value isn't a substring of the condition value. As said above, selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – with C<Select>, C<Combobox> or C<Autocomplete> type are not eligible for this operator. Also, C<Date> and C<DateTime> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> are not eligible for this operator.

=item * C<less than>

The condition is met if and only if the current value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/ObjectCustomField.html> is less than or equal to the value setup for this condition. The comparison is achieved according to some kind of L<natural sort order|https://en.wikipedia.org/wiki/Natural_sort_order>, that is: number values are compared as numbers, strings are compared alphabetically, insensitive to case and accents (C<a = á>, C<a = A>). Moreover, IP Adresses (IPv4 and IPv6) are expanded to be compared as expected. As said above, selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – with C<Select>, C<Combobox> or C<Autocomplete> type are not eligible for this operator.

=item * C<greater than>

The condition is met if and only if the current value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/ObjectCustomField.html> is greater than or equal to the value setup for this condition. The comparison is achieved according to some kind of L<natural sort order|https://en.wikipedia.org/wiki/Natural_sort_order>, that is: number values are compared as numbers, strings are compared alphabetically, insensitive to case and accents (C<a = á>, C<a = A>), and dates with or without times are compared chronogically. Moreover, IP Adresses (IPv4 and IPv6) are expanded to be compared as expected. As said above, selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – with C<Select>, C<Combobox> or C<Autocomplete> type are not eligible for this operator.

=item * C<between>

The condition is met if and only if the current value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/ObjectCustomField.html> is greater than or equal to the first value setup for this condition and is less than or equal to the second value setup for this condition. That means that when this operator is selected, two values have to be entered. The comparison is achieved according to some kind of L<natural sort order|https://en.wikipedia.org/wiki/Natural_sort_order>, that is: number values are compared as numbers, strings are compared alphabetically, insensitive to case and accents (C<a = á>, C<a = A>), and dates with or without times are compared chronogically. Moreover, IP Adresses (IPv4 and IPv6) are expanded to be compared as expected. As said above, selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> – with C<Select>, C<Combobox> or C<Autocomplete> type are not eligible for this operator.

=back

As an exception, C<IPAddressRange> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> are not eligible as condition L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, since there is not really any sense in comparing two ranges of IP addresses. C<IPAddress> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, combined with C<between> operator, should be sufficient for most cases checking whether an IP address is included in a range.

If the condition L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> is selectable – with C<Select>, C<Combobox> or C<Autocomplete> type – it can be multivalued. Then, the condition for an object is met as soon as the condition is met by at least one value of the L<instanciated conditioned by custom field|https://docs.bestpractical.com/rt/5.0.9/RT/ObjectCustomField.html> for this object.

If a L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> is based on another (parent) L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> which is conditioned by, this (child) L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> will of course also be conditioned by (with the same condition as its parent). Nevertheless, there is a caveheat in  display mode: the order matters! That is the parent L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> should have a lower sort order than the child L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>.

From version 0.07, the condition can be multivalued, that is: the conditioned L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> can be displayed/edited if the condition L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> has one of these values (In other words: there is an C<OR> bewteen the values of the condition). The condition L<custom field|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> can be a select custom field with values defined by L<CustomFieldValues|https://docs.bestpractical.com/rt/5.0.9/RT/CustomFieldValues.html> or an L<external custom field|https://docs.bestpractical.com/rt/5.0.9/extending/external_custom_fields.html>.
...
Version 0.99 is also a complete redesign, with API changed, but backward compatibility with previously configured L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, assuming the default condition operator is C<is>.

From version 1.13, C<HTML> L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> introduced in RT 5.0.3 can be defined as condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>.


=head1 RT VERSION

Works with RT 4.2 or greater but not with RT 6 (use v2x versions of this module for RT 6)
...
This is needed if your C<RT> installation directory is not C</opt/rt5/>, as documented in L<https://docs.bestpractical.com/rt/5.0.9/extensions.html>.
...
C<ConditionalCustomFields> adds a C<ConditionedBy> property, that is a condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, an operator and one or more values, along with the following methods, to conditioned by L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> objects:

=head2 SetConditionedBy CF, OP, VALUE

Set the C<ConditionedBy> property for this L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> object to L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> C<CF> with operator set to C<OP> and value set to C<VALUE>. C<CF> should be an existing L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> object or the id of an existing L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> object, or the name of an unambiguous existing L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> object. C<OP> should be C<is>, C<isn't>, C<match>, C<doesn't match>, C<less than>, C<greater than> or C<between>. C<VALUE> should be a string or an anonymous array of strings (for selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> or C<between> operator). Current user should have C<SeeCustomField> and C<ModifyCustomField> rights for this conditioned by L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> and C<SeeCustomField> right for the condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>. Returns (1, 'Status message') on success and (0, 'Error Message') on failure.
...
Returns the current C<ConditionedBy> property for this conditioned by L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> object as a hash with keys C<CF> containing the id of the condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, C<op> and C<vals> containing the condition operator as string, and the condition value as an array of strings (so we can store several values for selectable L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> or C<between> operator, but generally the C<vals> array includes only one string). If neither this conditioned by L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> nor one of its ancestor is conditioned by the C<CF> condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, that is: if their C<ConditionedBy> property is not (recursively) defined, returns C<undef>. Current user should have C<SeeCustomField> right for both this conditioned by L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> and the condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> which this L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> is conditioned recursively by. I<"Recursively"> means that this method will search for a C<ConditionedBy> property for this L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> object, then for the L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> this one is C<BasedOn>, and so on until it finds an ancestor C<Category> with a C<ConditionedBy> property or, the L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> which is being looked up, is not based on any ancestor C<Category>.
...
Also, C<ConditionalCustomFields> allows to set the C<ConditionedBy> property when creating L<CustomFields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomFields.html> from an F<initialdata> file, with one of the following syntaxes:
...
This examples creates a C<Select> condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, named C<Condition> and three conditioned by L<CustomFields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>. L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> C<Condition> should have the value C<Passed>, for L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> C<Conditioned with cf name and value> to be displayed or edited. L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> C<Condition> should not have the value C<Failed> for L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> C<Conditioned with cf id and value> to be displayed or edited. L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> C<Condition> should have one of the values C<Passed> or C<Schrödingerized> for L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> C<Conditioned with multiple values> to be displayed or edited.
...
The condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> that this new L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> should conditioned by. It can be either the C<id> or the C<Name> of a previously created L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>. This implies that the condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> should be declared before this one in the F<initialdata> file, or it should already exist. When C<ConditionedByCF> attribute is set, C<ConditionedBy> field should always also be set.

=item C<ConditonedBy>

The value as a C<string> of the condition L<CustomField|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html> defined by the C<ConditionedByCF> field (which is mandatory).
...
C<ConditionalCustomFields> comes with a fairly complete test suite. As for every L<RT extention|https://docs.bestpractical.com/rt/5.0.9/writing_extensions.html#Tests>, to run it, you will need a installed C<RT>, set up in L<development mode|https://docs.bestpractical.com/rt/5.0.9/hacking.html#Test-suite>.

But, since C<ConditionalCustomFields> operates dynamically to show or hide L<custom fields|https://docs.bestpractical.com/rt/5.0.9/RT/CustomField.html>, most of its magic happens in C<Javascript>. Therefore, the test suite requires a scriptable headless browser with C<Javascript> capabilities. In previous versions of C<ConditionalCustomFields>, you would also need to install L<PhantomJS|http://phantomjs.org/>, along with L<WWW::Mechanize::PhantomJS> and L<Selenium::Remote::Driver>. Unfortunately L<PhantomJS|http://phantomjs.org/> has been deprecated for years and was unable to cope with javascript librairies, mostly C<bootstrap.js>, used by C<RT 5> core. Therefore, the test suite has been reduced to include only javascript free tests.
